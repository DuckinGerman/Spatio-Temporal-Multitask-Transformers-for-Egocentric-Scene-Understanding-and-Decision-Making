# -*- coding: utf-8 -*-
"""
视频滑窗推理+可视化（bbox+属性+语义）
- 与训练/val 完全共享 dataset.pipeline 与 data_preprocessor
- seg: 当前为 6 类（0..5，不包含背景），可视化按 6 通道 logits 处理
- 输出 2 个视频：
...
def main():
    ...
    ap.add_argument("--seg-thr", type=float, default=0.550, help="pixel confidence threshold (logits only)")
    ap.add_argument("--seg-ema", type=float, default=0.70, help="EMA factor for prob map (logits only)")
    ap.add_argument("--min-area", type=float, default=0.01, help="min connected area ratio (logits only)")
    ...
    while True:
        ...
        if len(paths) == T and (frame_idx % args.stride == T - 1 or args.stride == 1):
            ...
            # -------------------------
            # Segmentation visualization (6-class, NO background)
            # IMPORTANT:
            #   - Prefer `seg` (argmax label map) if available, because it often carries IGNORE_INDEX for background.
            #   - Only fall back to `seg_logits` when `seg` is missing.
            #   - Never colorize pixels that are IGNORE_INDEX / low-confidence.
            # -------------------------
            meta = pred_ds.metainfo

            if seg_logits is not None:
                # seg is usually a (H,W) label map in resized/crop space.
                # It may already contain IGNORE_INDEX for background/void.
                seg_ori = warp_seg_to_ori(seg.astype(np.int32), meta)  # (H0,W0)

                # resize to current visualization frame size
                ...
                # valid pixels: 0..5 only; exclude IGNORE(255) and background id=6
                ...
                # seg_logits: (C,h,w), C should be 6 (no background channel)
                ...
                # Treat background pixels as IGNORE for overlay purposes.
                if C > SEG_BG_ID:
                    seg_pred[seg_pred == SEG_BG_ID] = IGNORE_INDEX

                # confidence mask: treat low-confidence pixels as background/void
                mask = (conf_np >= args.seg_thr)
                seg_pred[~mask] = IGNORE_INDEX

                # small area cleanup (per class 0..5) - only on confident pixels
...
CUDA_VISIBLE_DEVICES=1 PYTHONPATH=. python projects/vasco/demo_vasco.py \
  ...
  --checkpoint /home/tili/masterwork/transformer/mmdetection-3.3.0/work_dirs/vasco_swin_2d-adparallel/epoch_20.pth \
  --input /home/tili/masterwork/transformer/mmdetection-3.3.0/GX010102.MP4 \
  --output /home/tili/masterwork/transformer/mmdetection-3.3.0/GX010102-demo.MP4 \
  --T 8 
  --stride 1
  --force-choice straight

# Example (COCO dataset mode, uses images[].ego_choice):
CUDA_VISIBLE_DEVICES=1 PYTHONPATH=. python projects/vasco/demo_vasco.py \
  --config /home/tili/masterwork/transformer/mmdetection-3.3.0/projects/vasco/configs/vasco_swin2d_temporal.py \
  --checkpoint /home/tili/masterwork/transformer/mmdetection-3.3.0/work_dirs/vasco_swin_2d-adparallel/epoch_20.pth \
  --ann-json /path/to/train.json \
  --img-root /path/to/dataset_root \
  --video-id 1 \
  --output GX010061-demo.MP4 \
  --T 8 --stride 1
"""
# backup