...
val_pipeline = [
    dict(type='LoadMultiImagesFromFile'),
    dict(type='SeqResizeFlipPad', short_edge_choices=(1080,), max_long_edge=1600),
    dict(type='PackVascoInputs'),
]
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        temporal_window=5, temporal_span=20, test_mode=True,
...
val_evaluator = [
    ...
    dict(type='VascoActionDirMetric', iou_thr=0.6, num_actions=5, num_dirs=6, score_thr=0.05),
    dict(type='VascoSegMetric', num_classes=7, ignore_index=255),
]
...
model = dict(
    ...
    backbone_3d=dict(
        ...
        pretrain='/home/tili/masterwork/transformer/mmdetection-3.3.0/projects/vasco/configs/swin-small-p244-w877_in1k-pre_8xb8-amp-32x2x1-30e_kinetics400-rgb_20220930-e91ab986.pth',  # 你已有的
        window_size=(8, 7, 7),
        out_indices=(0, 1, 2, 3),
        with_cp=True,
    ...
    roi_head=dict(
        ...
        temporal_aggregator=dict(
            ...
            k=2, bias_last=0.5, max_offset_px=1.5,
            detach_prev=True, gamma_max=0.25, gamma_scale=1.0,
            consistency_weight=0.1, heads=4, layers=1
        ),
        action_dir_head=dict(
            type='ActionDirHead',
            in_channels=256, roi_feat_size=7, hidden_dim=1024, with_mlp=True,
            num_actions=5, num_dirs=6,
        ...
        semantic_head=dict(
            type='SemanticHead',
            in_channels=256, num_classes=7, ce_weight=1.0, dice_weight=1, ignore_index=255
        ),
        aux_detach=True,
    ),
    train_cfg=dict(
        rpn=dict(assigner=dict(type='MaxIoUAssigner', pos_iou_thr=0.7, neg_iou_thr=0.3, min_pos_iou=0.3),
    ...
    test_cfg=dict(
        ...
        rcnn=dict(score_thr=0.05, nms=dict(type='nms', iou_threshold=0.6), max_per_img=300)
    )
)
...
param_scheduler = [
    ...
    dict(type='MultiStepLR', milestones=[27, 33], gamma=0.1),
]

default_hooks = dict(
    checkpoint=dict(
        type='CheckpointHook', interval=10, by_epoch=True, max_keep_ckpts=2, save_last=True,
        save_best=['coco/bbox_mAP','vasco/action_acc','vasco/dir_acc','seg/mIoU'],
